...
if (!url || process.argv[3] !== '--reset-test-data')
  throw new Error(
    '専用のテスト用サーバーURLと --reset-test-data を指定してください。対象大会を切り替えます。',
  );
...
const send = async (command) => {
  const response = await fetch(`${url}/api/state`, {
    ...
    body: JSON.stringify({ expectedRevision: state.revision, command }),
  });
...
abort.abort();
const conflict = await fetch(`${url}/api/state`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    expectedRevision: revision,
    command: { type: 'show', scene: 'logo', phase: 'break' },
  }),
});
assert.equal(conflict.status, 409);
assert.equal((await conflict.json()).state.revision, state.revision);
const crossOrigin = await fetch(`${url}/api/state`, {
  method: 'POST',
...
const complete = async (id) => {
  ...
  for (const number of [1, 2])
    await send({ type: 'save-game', matchId: id, game: score(number), correction: '' });
};
for (const match of [...state.matches]) await complete(match.id);
await send({
  type: 'generate-finals',
  qualifiers: ['smoke-0', 'smoke-3', 'smoke-1', 'smoke-4'],
  justification: '',
});
...
let final = state.matches.find((m) => m.label === '決勝');
assert.equal(final.a, 'smoke-0');
assert.equal(final.b, 'smoke-3');
...
assert.ok(csv.includes('"\'=検証選手0"'));
assert.ok(csv.includes('"決勝"'));
assert.equal(csv.split('\r\n').length, 19);
const exported = await (await fetch(`${url}/api/export`)).json();
assert.equal(exported.revision, state.revision);
assert.equal(exported.audit.length, state.audit.length);
