  ...
  return (
    ...
    >
            ...
            <div>
              <input
                ...
                onChange={(event) => {
                  ...
                  set({ [side === 'a' ? 'scoreA' : 'scoreB']: Number(event.target.value) });
                }}
              />
              <span>点</span>
            </div>
        ...
        {game.reason !== 'points' && (
          <label>
            ...
            <select
              ...
              onChange={(event) => set({ specialWinner: event.target.value as 'a' | 'b' })}
            >
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => {
                ...
                set({ remainingTurns: Number(event.target.value) });
              }}
            />
          </label>
        )}
      ...
      <div className="calculation-preview">
        <span>この戦の判定</span>
        <strong>
          {!scoreInputs.a || !scoreInputs.b
            ? '両者の得点を入力してください'
            : result.winner
              ? `${playerName(state, match[result.winner])} の勝利`
              : game.reason === 'points'
                ? '同点'
                : '勝者を選択してください'}
        </strong>
...
export default function MatchEditor({
  ...
  const [replayOpen, setReplayOpen] = useState(false),
    [replayReason, setReplayReason] = useState('');
  ...
  return (
    <div className="match-editor">
      ...
      {match.bye ? (
        <p className="muted">不戦進出として次の試合へ反映済みです。</p>
      ) : !match.a || !match.b ? (
        <div className="empty-state">前の試合の勝者が確定すると、対戦者が入ります。</div>
      ) : !match.started ? (
        <form
        ...
        >
          ...
          <label>
            ...
            <select
              required
              value={firstCool}
              onChange={(event) => setFirstCool(event.target.value)}
            >
              <option value="" disabled>
                参加者を選択
              </option>
          ...
          <div className="game-tabs">
            {([1, 2] as const).map((n) => (
              <button
                type="button"
                key={n}
                className={number === n ? 'active' : ''}
                onClick={() => setNumber(n)}
                disabled={n === 2 && !match.games.some((g) => g.number === 1)}
              >
                第{n}戦{' '}
                <span>{match.games.some((g) => g.number === n) ? '記録済み ✓' : '未記録'}</span>
              </button>
            ))}
          </div>
          {locked ? (
            <div className="notice">
              本戦の組み合わせ確定済みのため、予選結果は保護されています。
            </div>
          ) : (
            <GameForm
              key={`${match.id}-${match.attempt}-${number}-${JSON.stringify(match.games.find((g) => g.number === number))}`}
              state={state}
              match={match}
              number={number}
              send={send}
              onSaved={() => {
                if (number === 1 && !match.games.some((g) => g.number === 2)) setNumber(2);
              }}
            />
          )}
          {!locked && (
