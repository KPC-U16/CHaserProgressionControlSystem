...
export function readState(): ControlState {
  if (!existsSync(dataFile)) { const state = createInitialState(); writeState(state); return state; }
...
export function dispatch(command: Command, expectedRevision: number): ControlState {
  ...
  if (previous.revision !== expectedRevision) throw new RevisionConflict('別の操作で更新されました。最新の表示を確認してから、もう一度操作してください。');
  const next = stateSchema.parse(applyCommand(previous, command));
  const backup = command.type === 'reset' ? `archive-${Date.now()}-${previous.revision}.json` : 'state.previous.json';
