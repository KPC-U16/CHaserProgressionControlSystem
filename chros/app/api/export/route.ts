...
export function GET(request: Request) {
  ...
  for (const match of state.matches) {
    ...
    const attempts = [
      ...match.previousAttempts.map((attempt, i) => ({ ...attempt, number: i + 1, active: false })),
      {
        games: match.games,
    ...
    for (const attempt of attempts)
      for (const game of attempt.games) {
        const cool =
          game.number === 1 ? attempt.firstCool : attempt.firstCool === match.a ? match.b : match.a;
        ...
        rows.push([
          ...
          game.number,
          playerName(state, cool),
          playerName(state, hot),
          game.scoreA,
          game.scoreB,
          ...
          attempt.active && result.winnerId ? playerName(state, result.winnerId) : '',
          attempt.active && result.complete ? (result.tied ? '再試合待ち' : result.decidedBy) : '',
          attempt.reason,
          state.profile,
          state.ruleVersion,
        ]);
  ...
  const cell = (value: string | number) =>
    `"${(typeof value === 'number' ? String(value) : value.replace(/^[=+\-@\t\r]/, "'$&")).replaceAll('"', '""')}"`;
  return new Response(
    csv
      ...
      : JSON.stringify(state, null, 2),
    {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="chros-${new Date().toISOString().slice(0, 10)}.${csv ? 'csv' : 'json'}"`,
