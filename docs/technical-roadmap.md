...
/// 事実と裁定からスコア算出ブロックの「入力の箱」を構成する（§5.4）。
model Score {
  id            Int   @id @default(autoincrement())
  matchId       Int
...
- `MatchResult.winReason`: 「どのような勝ち方をしたか」の表示要件（スコア表示）に直接対応する。
  現行は `put` / `lostConnect` から表示側が推論する必要があり、ロジックが散る。
  値は enum ではなく文字列とする。勝因は規則側の終端ノード構成で決まるため、
  システム側で列挙し切れない。
- `MatchResult.agent*Output` を `Json` にする理由: **何をスコアとして外に出すかは規則が決める**（§5.3）。
  予選スコア1つの規則もあれば勝利数と補正アイテム数を出す規則もあるため、固定カラムにできない。
...
| 用語 | 指すもの | 例 |
| --- | --- | --- |
| **ブロック** | 編集単位。ユーザーが追加・削除できない固定の区画 | スコア算出ブロック、勝敗判定ブロック |
| **ノード** | ブロック内に置く処理の1単位。ユーザーが自由に配置する | 加算ノード、同値判定ノード |
| **ポート** | ノードの入出力の接続口。名前と型を持つ | `left` / `right` / `yes` / `no` |
| **値エッジ** | 数値・真偽値を運ぶ線。スコア算出ブロックのみ | 加算ノード → 出力スロット |
...
**スコア算出ブロックの定義は1つだけ持ち、評価時に両参加者へ適用する。**
2ブロックに見えるのは評価時の姿であり、編集対象は1つ。
両者に同じ規則が当たることを構造で保証でき、「片方だけ直し忘れて不公平になる」バグが起きない。
先攻/後攻による差はブロックを分けるのではなく、入力（前半/後半、COOL/HOT）を参照して
ブロック内の分岐として表現する。

上図は先後交代2対戦の例。Bot予選は1対戦のスコア算出と順位への出力で足り、
相手Botの換算スコアと比較して試合勝敗を決めるブロックを付けない。
対戦勝敗は前段の裁定結果を用いる。`purpose` と `inputShape` で両者を区別する。

この分割が持つ意味:

- **「何点になるか」と「どちらが勝ちか」を混ぜない。** スコア算出ブロックは数値を出すことだけに責任を持ち、
  勝敗の概念を知らない。勝敗判定ブロックはスコアの作り方を知らない。
  規則変更の多くは片方だけの修正で済む。
- スコア算出ブロックは**片側ぶんの入力しか受け取れない**。相手のスコアを参照できないため、
  「相手より1点多ければ」のような比較は構造上ここに書けず、必ず勝敗判定ブロックに寄る。
  この制約が、規則の置き場所の曖昧さを消す。
- 勝敗判定ブロックの出力は勝者と勝因のみ。スコアをここで書き換えることはできない。
...
/** スコア算出ブロック: 値エッジのみの純粋な式。複数の値を名前付きで外に出す */
...
スコア算出ブロックの出力スロットが選択肢として並び、その中から1つ選ぶ。
...
値エッジを持つのはスコア算出ブロックだけになる。§5.2 の用語表では2種類の線を定義しているが、
...
同じ語彙のまま、分岐を置く順を入れ替えるだけで「スコアが先、要因が後」の大会にも対応できる。
...
制御フローを持つのは勝敗判定ブロックだけであり、スコア算出ブロックには制御エッジを定義しない。
スコア算出は常に全ノードを評価して出力スロットを埋める純粋な式のままとする。
...
対戦勝者と参加者のスコアを残し、Botを順位対象から除くのはステージ側の責務とする。

### 5.4 入力の箱

スコア算出ブロックが参照できる入力を「箱」として定義する。
...
§5.3 で「スコア算出ブロックは自分の値のみを参照できる」と述べたが、正確には次のとおり。

- **参照できない**: 相手の**出力スロット**（相手の算出済みスコア）。
  これを許すとスコア算出が相互参照になり、対称性の保証とブロック分割の意味が失われる。
...
それはスコア算出ブロックで組み立てるものであり（§5.5 のBot予選）、
...
- 釧路は原因が後半だけでも新マップで前後半ともやり直し、前半のスコアを持ち越さない。
...
3. **スロット参照検証**: 勝敗判定ブロックの各ノードが選んでいる `slot` が、
   スコア算出ブロックに実在し、かつノードの要求する型と一致すること
...
4. **型検証**（スコア算出ブロックのみ）: 値エッジのポート間の型（数値 / 真偽値）の整合。
...
特に相手Putと自滅の補正差、Bot戦の負スコア、釧路の後半障害による全試合無効化、
...
- スコア算出ブロック: 各ノードの中間値（なぜこの点数になったか）
...
Bot予選にはスコア算出のtraceだけがあり、前段の原因分類・裁定の参照は別途共通で残す。
...
- **分岐ノードの追加**: 初版は「同値判定」「フラグ判定」の2つ。
  ...
  §5.5の旭川の計算例では真偽値をスコア算出の `if` で消費できる。
...
- **引き分けの扱い**: 「引き分け」終端に到達した対戦を、
  ...
  Bot戦の対戦自体が同点となった場合や予選の同スコア順位は、対人試合の再試合とは別に確認する。
...
`Stage.format` で描き分ける。Bot予選は参加選手とBot・スコアの一覧とする。
...
- 要確認: 旭川のBot戦が同点の場合、予選スコアが同じ場合、通過人数・組み合わせ。
